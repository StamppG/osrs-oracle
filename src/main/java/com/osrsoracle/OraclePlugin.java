...
public class OraclePlugin extends Plugin
{
	...
	/*
	 * Keep the old visual 124-page scanner in the file as a fallback,
	 * but disable automatic startup while testing the instant path.
	 */
	private static final boolean ENABLE_LEGACY_COLLECTION_PAGE_SCANNER = false;
	...
	/*
	 * FULL COLLECTION LOG SCANNER
	 *
	 * Important: Collection Log navigation widgets are not guaranteed to
	 * remain queryable after entering a detail page. Cache the raw menu
	 * action metadata first, then replay those cached actions.
	 */
	private boolean collectionFullScanStarted = false;
	private boolean collectionFullScanFinished = false;

	private int collectionCategoryIndex = 0;
	private int collectionPageIndex = 0;

	private int collectionScanDelayTicks = 0;
	private int collectionScanWaitTicks = 0;
	private int collectionScanRetries = 0;

	private String collectionExpectedPage = null;

	private boolean collectionNeedCategoryClick = false;
	private boolean collectionNeedPageListCache = false;
	private boolean collectionFastAdvanceQueued = false;
	private long collectionLastPageOpenNanos = 0L;
	private int collectionLastPageOpenTick = -1;

	private final List<CollectionNavAction> collectionCategoryActions =
			new ArrayList<>();

	private final List<CollectionNavAction> collectionCurrentPageActions =
			new ArrayList<>();

	private static final int[] COLLECTION_CATEGORY_CHILD_IDS = {
			4, 5, 6, 7, 8
	};

	private static final String[] COLLECTION_CATEGORY_NAMES = {
			"Bosses",
			"Raids",
			"Clues",
			"Minigames",
	...
	/*
	 * Proven actionable page-list containers for each category:
	 * Bosses=11, Raids=15, Clues=32, Minigames=27, Other=34.
	 */
	private static final int[] COLLECTION_PAGE_LIST_CHILD_IDS = {
			11, 15, 32, 27, 34
	};

	private static class CollectionNavAction
	{
	...
	public void onGameTick(GameTick gameTick)
	{
		 ...
		 */
		if (
				collectionFullScanStarted &&
						!collectionFullScanFinished
		)
		{
			if (
					collectionCategoryIndex >=
							collectionCategoryActions.size()
			)
			{
				collectionFullScanFinished = true;

				log.info(
						"CLOG FULL SCAN COMPLETE: cachedPages={}",
						cachedCollectionLogPages.size()
				);
			}
			else if (collectionNeedCategoryClick)
			{
				collectionScanDelayTicks++;

				if (collectionScanDelayTicks >= 1)
				{
					CollectionNavAction category =
							collectionCategoryActions.get(
									collectionCategoryIndex
							);

					log.info(
							"CLOG FULL SCAN: opening category {}/{} name='{}'",
					...
					collectionNeedCategoryClick = false;
					collectionNeedPageListCache = true;
					collectionCurrentPageActions.clear();
					collectionPageIndex = 0;
					collectionExpectedPage = null;
					collectionScanDelayTicks = 0;
					collectionScanWaitTicks = 0;
					collectionScanRetries = 0;
				}
			}
			else if (collectionNeedPageListCache)
			{
				int pageListChildId =
						COLLECTION_PAGE_LIST_CHILD_IDS[
								collectionCategoryIndex
								];
				...
				boolean pageListReady =
						pageList != null &&
								!pageList.isHidden() &&
								pageList.getChildren() != null &&
								pageList.getChildren().length > 0;
				...
				if (pageListReady)
				{
					...
					for (Widget page : pages)
					{
						...
						collectionCurrentPageActions.add(
								new CollectionNavAction(
										page.getIndex(),
										pageList.getId(),
										"Check",
										page.getName(),
					...
					if (collectionCurrentPageActions.isEmpty())
					{
						collectionScanWaitTicks++;
					}
					else
					{
						log.info(
								"CLOG FULL SCAN: cached category='{}' child={} pages={}",
								collectionCategoryActions.get(
										collectionCategoryIndex
								).cleanName,
								pageListChildId,
								collectionCurrentPageActions.size()
						);

						collectionNeedPageListCache = false;
						collectionPageIndex = 0;
						collectionScanDelayTicks = 0;
						collectionScanWaitTicks = 0;
						collectionScanRetries = 0;
				...
				else
				{
					...
					if (collectionScanWaitTicks == 2)
					{
						log.info(
								"CLOG FULL SCAN: waiting for category='{}' child={} to become visible",
								collectionCategoryActions.get(
										collectionCategoryIndex
								).cleanName,
								pageListChildId
				...
				if (
						collectionNeedPageListCache &&
								collectionScanWaitTicks >= 20
				)
				{
					log.info(
							"CLOG FULL SCAN: category list unavailable category='{}' child={}; skipping category",
							collectionCategoryActions.get(
									collectionCategoryIndex
							).cleanName,
							pageListChildId
					...
					collectionExpectedPage = null;
					collectionNeedPageListCache = false;
					collectionNeedCategoryClick = true;
				}
			}
			else if (collectionExpectedPage != null)
			{
				collectionScanWaitTicks++;

				if (collectionScanWaitTicks >= 10)
				{
					if (collectionScanRetries < 1)
					{
						collectionScanRetries++;
						collectionScanWaitTicks = 0;
						collectionScanDelayTicks = 0;

						CollectionNavAction page =
								collectionCurrentPageActions.get(
										collectionPageIndex
								);

						log.info(
								"CLOG FULL SCAN: retrying category='{}' page='{}'",
								collectionCategoryActions.get(
										collectionCategoryIndex
								).cleanName,
								page.cleanName
					...
					{
						log.info(
								"CLOG FULL SCAN: skipping timed-out category='{}' pageIndex={} expected='{}'",
								collectionCategoryActions.get(
										collectionCategoryIndex
								).cleanName,
								collectionPageIndex,
								collectionExpectedPage
						);
			...
			{
				log.info(
						"CLOG FULL SCAN: category complete name='{}' pages={}",
						collectionCategoryActions.get(
								collectionCategoryIndex
						).cleanName,
						collectionCurrentPageActions.size()
			...
			else
			{
				 ...
				 * Fallback opener.
				 *
				 * Normal page-to-page advancement is now queued immediately
				 * after the expected page is captured. This game-tick path is
				 * retained as a safety net in case no fast advance is queued.
				 */
				if (!collectionFastAdvanceQueued)
				{
					collectionScanDelayTicks++;

					if (collectionScanDelayTicks >= 1)
					{
						openCurrentCollectionPage();
					}
				}
			}
		}
	...
	)
	{
		 ...
		 * redraw spam. During an active full scan, however, that 500 ms
		 * throttle becomes the dominant speed limit (about 2 pages/sec).
		 *
		 * The full scanner already has a stronger safety gate: it will not
		 * advance until COLLECTION_DRAW_LIST fires for the expected page.
		 * So bypass the time throttle only while a full scan is active.
		 */
		boolean fullScanActive =
				collectionFullScanStarted &&
						!collectionFullScanFinished;

		if (
				!fullScanActive &&
						now - lastCollectionCaptureTime < 500
		)
		{
			return;
		}
		...
		/*
		 * Start the full scan after a real Collection Log page has rendered.
		 * Before navigating anywhere, cache all five category actions while
		 * those widgets are known to exist.
		 */
		if (
				ENABLE_LEGACY_COLLECTION_PAGE_SCANNER &&
						!collectionFullScanStarted &&
						!collectionFullScanFinished
		)
		{
			collectionCategoryActions.clear();

			for (
					int i = 0;
					i < COLLECTION_CATEGORY_CHILD_IDS.length;
					i++
			)
			{
				Widget category =
						client.getWidget(
								621,
								COLLECTION_CATEGORY_CHILD_IDS[i]
						);

				if (category == null)
				{
					log.info(
							"CLOG FULL SCAN: category cache missing index={} name='{}'",
							i,
							COLLECTION_CATEGORY_NAMES[i]
					);

					continue;
				}

				collectionCategoryActions.add(
						new CollectionNavAction(
								category.getIndex(),
								category.getId(),
								"View",
			...
			if (collectionCategoryActions.isEmpty())
			{
				log.info(
						"CLOG FULL SCAN: no category actions could be cached; scan not started"
				);
			}
			else
			{
				collectionFullScanStarted = true;
				collectionCategoryIndex = 0;
				collectionPageIndex = 0;
				collectionScanDelayTicks = 0;
				collectionScanWaitTicks = 0;
				collectionScanRetries = 0;
				...
				log.info(
						"CLOG FULL SCAN: cached {} category actions; started after initial page='{}'",
						collectionCategoryActions.size(),
						pageName
				);
			}
		...
		/*
		 * Advance only when the page requested by the scanner has actually
		 * rendered and been captured.
		 */
		if (
				collectionExpectedPage != null &&
						collectionExpectedPage.equals(
								pageName
						)
		)
		{
			long captureNowNanos = System.nanoTime();
			long elapsedMs =
					collectionLastPageOpenNanos > 0L
							? (captureNowNanos - collectionLastPageOpenNanos) / 1_000_000L
							: -1L;

			log.info(
					"CLOG TIMING: CAPTURE tick={} openedTick={} elapsedMs={} category='{}' page='{}'",
					client.getTickCount(),
					collectionLastPageOpenTick,
					elapsedMs,
					collectionCategoryActions.get(
							collectionCategoryIndex
					).cleanName,
			 ...
			 * FAST PAGE ADVANCE
			 *
			 * The old scanner deliberately waited for the next 600 ms game
			 * tick before clicking the next page. We already know this page
			 * rendered successfully because COLLECTION_DRAW_LIST fired and
			 * the expected page was captured above.
			 *
			 * Queue the next click for the next client-thread cycle instead.
			 * This keeps the same render/capture gate, but removes almost all
			 * of the artificial per-page delay.
			 */
			if (
					collectionPageIndex <
							collectionCurrentPageActions.size()
			...
			{
				queueFastCollectionAdvance();
			}
		}
	}


	private void queueFastCollectionAdvance()
	{
		if (
				collectionFastAdvanceQueued ||
						!collectionFullScanStarted ||
						collectionFullScanFinished ||
						collectionNeedCategoryClick ||
						collectionNeedPageListCache ||
						collectionExpectedPage != null ||
						collectionPageIndex >=
								collectionCurrentPageActions.size()
		...
		collectionFastAdvanceQueued = true;

		clientThread.invokeLater(
				() ->
				{
					collectionFastAdvanceQueued = false;
	...
	private void openCurrentCollectionPage()
	{
		if (
				collectionPageIndex < 0 ||
						collectionPageIndex >=
		...
		collectionLastPageOpenNanos = System.nanoTime();
		collectionLastPageOpenTick = client.getTickCount();

		log.info(
				"CLOG TIMING: OPEN tick={} category='{}' page {}/{} name='{}'",
				collectionLastPageOpenTick,
				collectionCategoryActions.get(
						collectionCategoryIndex
				).cleanName,
